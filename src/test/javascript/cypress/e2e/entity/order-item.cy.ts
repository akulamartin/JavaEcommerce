...
describe('OrderItem e2e test', () => {
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"infrastructures","description":"background Baby","price":50830,"thesize":"XXL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2023-02-21T10:10:41.469Z","status":"PENDING","code":"salmon Junctions"},
    }).then(({ body }) => {
      productOrder = body;
    });
  });
